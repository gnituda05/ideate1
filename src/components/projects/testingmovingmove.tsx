...
const ScrollDown1 = () => {
  ...
  const handleScroll = (event: React.UIEvent<HTMLDivElement>) => {
    // Asserting the type to HTMLElement
    const target = event.currentTarget as HTMLElement;

    // Check if the user has scrolled down
    setScrolledDown(target.scrollTop > 0); // Adjust the threshold as needed
  };

  const panelStart = 600; // Position where the panel starts moving
  const panelEnd = 600;
  const popUpStart = 600;
  ...
  return (
    <div className="  bg-gray-800 h-screen overflow-hidden ">
      <div className="flex flex-col mx-auto max-w-screen-lg gap-6 ">
        <div
          className="relative group h-screen overflow-auto"
