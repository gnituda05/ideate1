...
const Header: React.FC<HeaderProps> = ({ currentPage, onNavigate1 }) => {
  ...
  useEffect(() => {
    if (currentPage === "Home") {
      const handleScroll = () => {
        const currentScrollY = window.scrollY;
        setIsShrunk(currentScrollY > window.innerHeight / 2);
      };
  ...
  const handleNavigate = (e: React.MouseEvent<HTMLElement>, target: string) => {
    e.preventDefault();
    onNavigate1(target);
